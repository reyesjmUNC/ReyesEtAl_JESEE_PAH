function [] = getMaps(pah,LR,soft,showhard,showsoft)
...
for i = 1:length(dayShow)
    ...
    if soft == 1
        title(sprintf('Mean %s ellip %s %s on %s',valdispname{pah}, ...
            LRstr,hardstr,datestr(dayShow(i),2)));
    else
        title(sprintf('Mean %s kriging on %s',valdispname{pah},datestr(dayShow(i),2)));
    end
    ...
    if soft == 1
        print(gcf,'-painters','-dpng','-r600', ...
            sprintf('figures/BMEest_%s_ellip_%s_%s_%s.png', ...
            valname{pah+4},LRstr,hardstr,datestr(dayShow(i))));
    else
        print(gcf,'-painters','-dpng','-r600', ...
            sprintf('figures/BMEest_%s_%s_%s_%s.png', ...
            valname{pah+4},LRstr,hardstr,datestr(dayShow(i))));
    end
