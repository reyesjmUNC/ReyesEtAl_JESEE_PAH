function [moments,info]=BMEprobaMoments2_parallel(ck,ch,cs,zh,softpdftype,nl,limi,probdens,covmodel,covparam,nhmax,nsmax,dmax,order,options);
...
parfor i=1:nk,
  ...
  [chlocal,zhlocal,dh,sumnhlocal]=neighbours2a(ck0,ch,zh,nhmax,dmax);
  [cslocal,nllocal,limilocal,probdenslocal,ds,sumnslocal]=...
    probaneighbours2a(ck0,cs,softpdftype,nl,limi,probdens,nsmax,dmax);
...
%     moments(i,:)=0;
%     momentstemp = zhlocal(index(2));
%     moments(i,1)=momentstemp;              % return the hard data value as the mean estimate
...
%   end;
  ...
  if iscomputed==0
    ...
    if ~isduplicate
      ...
      [BMEmean,stdDev,skewCoef,infotemp]=momentsFun(zhlocal,softpdftype,...
        nllocal,limilocal,probdenslocal,options,BsIFh,KsIFh,BkIFhs,KkIFhs);
      info(i,:) = infotemp;
      moments(i,:) = [ BMEmean+mkest stdDev^2 skewCoef ]; % mkest added back
    else
      Kks=[[Kk,Kk_s];[Kk_s',Ks]];
      if sumnhlocal==0
        BksIFh=[];
      ...
      [BMEmean,stdDev,skewCoef,infotemp]=momentsDupFun(zhlocal,softpdftype,...
